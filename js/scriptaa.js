...
/*
    ...
    console.log("adsf");
    $( "#modal-confirm-delete-category" ).modal({
      autoOpen:false,
      resizable: false,
      height: "auto",
      width: 400,
      ...
      buttons: {
        "Delete": function() {
          ...
          $.ajax({
              ...
              success: function(d)
               {
                   if(d == "1") //there's children - can't delete
                   {
                       $("#modal").html("This category has children.  It cannot be deleted.").modal("show");
                       console.log("it has children");
                   }
                   else
                   {
...
*/
    ...
    $("#cats").on("click", ".deleteLink", function(e) {
        e.preventDefault();
        data = $(this).parent().attr("id").substring(3, 5);
        theThis = $(this);
    ...
    $("#cats").on("click", ".editLink", function(e) {
        ...
        if($(this).html() == "Edit")
        {
            var catTitle = $(this).parent().find("span.catTitle").first().html();
            $(this).parent().find("span.catTitle").first().html("<input class=\"\" type=\"text\" value=\"" + catTitle + "\" style=\"width:" + (catTitle.length*10+30) + "px;\"/>");
            $(this).html("Save");
...
function saveEditedCat(theThis) {
    uid = theThis.parent().attr("id").substring(3, 5);
    var newVal = theThis.prev().children("input").val();
    var clicker = theThis;
    $.ajax({
    url: "process.php",
    data: "action=editcat&uid=" + uid + "&category=" + newVal,
    success: function(d)
       {
           if(d == "1")
           {
    ...
    success: function(d)
       {
           ...
           if (Object.keys(results).length > 0) {
               ...
               $.each(results, function(key, value)
               {
                ...
                catSelector.append("<div class=\"category item\" id=\"cat" + value[0] + "\" style= \"margin-left:"+ (value[1] * 5) +"px\" >" + output + "<span class=\"catTitle\">" + key + "</span>&nbsp;<a class=\"editLink\" href=\"" + value[0] + "\">Edit</a> <a class=\"deleteLink\" href=\"" + value[0] + "\">Delete</a><div class=\"links\"></div>");
